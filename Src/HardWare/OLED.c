...
{
	IIC_SendHead(0x78,0x00);	//写命令
	IIC_AByteSend(Command); 
	IIC_SendEnd();
}
/**
	* @brief  OLED多次写不同数据
...
{
	IIC_SendHead(0x78,0x40);//写数据
	IIC_BytesSend(Data,num,start,ifsame);
	IIC_SendEnd();
}
...
/**
  ...
  * @param  y 以左上角为原点，向下方向的坐标，范围：0~63
	...
	* @param  h 要上屏区域高度，范围：1~64
  */
void OLED_InverseArea(uint8_t x, uint8_t y, uint8_t width, uint8_t height)
{
	uint8_t PageStart = y / 8;									//始页数（0~7）
	uint8_t Pagend = (y + height - 1) / 8;			//终页数（0~7）
	uint8_t Yshift = y % 8;											//上部残卷空白量（填满0~7）
	uint8_t Endfull = (y + height - 1) % 8 + 1;	//下部残卷填充量（1~8填满）
	uint8_t InverseTemp[8][128] = {0};
	if(Yshift)//1：上部未填满；0上部填满不为残卷
	{
		for(uint8_t i = Yshift; i < 8; i++)
		{
			for(uint8_t j = x; j < x + width; j++)
			{
				InverseTemp[PageStart][j] = (ImageTransBuff[PageStart][j] ^ (0x01 << i));
			}
		}
		PageStart ++;
	}
	...
	for(uint8_t i = PageStart; i <= Pagend; i++)//中间整卷
	{
		for(uint8_t j = x; j < x + width; j++)
		{
			InverseTemp[i][j] = (ImageTransBuff[PageStart][j] ^ 0xFF);
		}
	}
	//上屏
	for(uint8_t i = PageStart; i <= Pagend; i++)
	{
		OLED_SetCursor(i, x);
		OLED_WriteDataS(InverseTemp[i], width, x, DISABLE);
	}
}
/**
  * @brief  OLED显示一张图片
  ...
  */
ErrorStatus OLED_ShowImg(uint8_t Xpos, uint8_t Ypos, uint8_t Width, uint8_t Height, FunctionalState ifmove, FunctionalState ifup, const uint8_t *Image)
{
...
/**
  ...
  * @retval 无
  */
void OLED_ShowChar(uint8_t Line, uint8_t Column, char Char, FunctionalState ifup, FunctionalState FontSize)
{      	
	if(FontSize)
	{
		OLED_ShowImg((Column - 1) * 8, (Line - 1) * 16, 8, 16, ifup, ifup, OLED_F8x16[Char - ' ']);
	}
	else
	{
		OLED_ShowImg((Column - 1) * 6, (Line - 1) * 8, 6, 8, ifup, ifup, OLED_F6x8[Char - ' ']);
	}
}
...
/**
  ...
  * @retval 无
  */
void OLED_ShowString(uint8_t Line, uint8_t Column, char *String, FunctionalState ifup, FunctionalState FontSize)
{
	uint8_t i;
	for (i = 0; String[i] != '\0'; i++)
	{
		OLED_ShowChar(Line, Column + i, String[i], ifup, FontSize);
	}
}
...
/**
  ...
  * @retval 无
  */
void OLED_ShowNum(uint8_t Line, uint8_t Column, uint32_t Number, uint8_t Length, FunctionalState ifup, FunctionalState FontSize)
{
	uint8_t i;
	for (i = 0; i < Length; i++)							
	{
		OLED_ShowChar(Line, Column + i, Number / OLED_Pow(10, Length - i - 1) % 10 + '0', ifup, FontSize);
	}
}
//去前面的零
void OLED_ShowCNum(uint8_t Line, uint8_t Column, uint32_t Number, uint8_t Length, FunctionalState ifup, FunctionalState FontSize)
{
	uint8_t i=0;
			while((Number / OLED_Pow(10, Length - i - 1) % 10) == 0 && i < Length-1)
			{
				OLED_ShowChar(Line, Column + i, ' ', ifup, FontSize);
				i++;
			}
			for(; i < Length; i++)
			{
...
/**
  ...
  * @param  Point 要显示小数点位置，范围：1~Length-1
	* @retval 无
	*/
void OLED_ShowRPNum(uint8_t Line, uint8_t Column, uint32_t Number, uint8_t Length, uint8_t Point, FunctionalState ifup, FunctionalState FontSize)
{
	uint8_t i = 0;
	...
	OLED_ShowChar(Line, Column + i,'.', ifup, FontSize); i++;
	for (; i < Length + 1; i++)							
	{
		OLED_ShowChar(Line, Column + i, Number / OLED_Pow(10, Length - i) % 10 + '0', ifup, FontSize);
	}
}
...
/**
  ...
  * @retval 无
  */
void OLED_ShowSignedNum(uint8_t Line, uint8_t Column, int32_t Number, uint8_t Length, FunctionalState ifup, FunctionalState FontSize)
{
	uint8_t i;
	uint32_t Number1;
	if (Number >= 0)
	{
		OLED_ShowChar(Line, Column, '+', ifup, FontSize);
		Number1 = Number;
	}
	else
	{
	...
	for (i = 0; i < Length; i++)							
	{
		OLED_ShowChar(Line, Column + i + 1, Number1 / OLED_Pow(10, Length - i - 1) % 10 + '0', ifup, FontSize);
	}
}
...
/**
  ...
  * @retval 无
  */
void OLED_ShowHexNum(uint8_t Line, uint8_t Column, uint32_t Number, uint8_t Length, FunctionalState ifup, FunctionalState FontSize)
{
	...
	for (i = 0; i < Length; i++)							
	{
		...
		if (SingleNumber < 10)
		{
			OLED_ShowChar(Line, Column + i, SingleNumber + '0', ifup, FontSize);
		}
		else
		{
			OLED_ShowChar(Line, Column + i, SingleNumber - 10 + 'A', ifup, FontSize);
		}
	}
}
...
/**
  ...
  * @retval 无
  */
void OLED_ShowBinNum(uint8_t Line, uint8_t Column, uint32_t Number, uint8_t Length, FunctionalState ifup, FunctionalState FontSize)
{
	uint8_t i;
	for (i = 0; i < Length; i++)							
	{
		OLED_ShowChar(Line, Column + i, Number / OLED_Pow(2, Length - i - 1) % 2 + '0', ifup, FontSize);
	}
}
...
void OLED_ShowChineseN(uint8_t Line, uint8_t Column, FunctionalState ifup, char *ChineseChar)//输出自己的文字库
{      	
	...
	for(uint8_t i = 0; ChineseChar[i] != '\0'; i++)//汉字字符串编码遍历
	{
		...
		if(indexTemp >= 3)//字符串分割单字编码
		{
			...
			OLED_ShowImg(Column*8 + lenght*16 - 24, Line*8 - 8, 16, 16, 0, ifup, OLED_CF16x16[indexTemp].FontData);
			indexTemp = 0;
		}
	}
}
...
{	
	...
	while(1)
	{
		OLED_ShowImg(0,0,128,64,DISABLE,ENABLE,Image_desk);
		OLED_BuffUpdata(0,0,128,64,1);
		Delay_ms(3000);
		OLED_Clear();
		OLED_ShowString(FLINEMIDDLE,FCOLRIGHT_6X(13),"123!@#$%^&*()",ENABLE,FONTSIZE_6X8);
		OLED_ShowString(FLINEMIDDLE+1,FCOLMIDDLE_6X(13),"123!@#$%^&*()",ENABLE,FONTSIZE_6X8);
		OLED_ShowString(1,FCOLMIDDLE_8X(12),"abcdEFG_123!",ENABLE,FONTSIZE_8X16);
		...
		Delay_ms(3000);
		OLED_Clear();
		OLED_ShowString(1,FCOLRIGHT_8X(12),"abcdEFG_123!",ENABLE,FONTSIZE_8X16);
		OLED_ShowString(FLINEUPS16_X8(2),FCOLRIGHT_6X(13),"123!@#$%^&*()",ENABLE,FONTSIZE_6X8);
		OLED_ShowString(FLINEDNS16_X8(3),FCOLRIS16_6X(6)-12,"123!@#$%^&*()",ENABLE,FONTSIZE_6X8);
		OLED_ShowChineseN(FLINEDNS16_X8(2),FCOLMIDDLE_16(6),ENABLE,"显示调试结果");
